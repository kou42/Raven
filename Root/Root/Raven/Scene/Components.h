...
struct AnimatorComponent
{
    ...
    // StateMachineは任意です。単一Clipを直接Animator::Play()する既存経路はそのまま利用でき、
    // Characterなど複数Stateを持つEntityだけStateMachineを設定します。
    // StateMachine内部はAnimatorを参照するため、ComponentがAnimatorとStateMachine双方を
    // shared_ptrで保持し、AnimatorのLifetimeがStateMachineより短くならないようにします。
...
struct RigidBodyComponent
{
    ...
    {
        if (Type == BodyType::Static || mass <= 0.0f) { Mass = 0.0f; InverseMass = 0.0f; return; }
    ...
    void SetBodyType(BodyType type)
    {
        ...
        {
            InverseMass = 0.0f; LinearVelocity = math::Vec3{}; AngularVelocity = math::Vec3{};
            Force = math::Vec3{}; Torque = math::Vec3{}; IsSleeping = true; SleepTimer = 0.0f;
        }
        else
        {
            InverseMass = Mass > 0.0f ? 1.0f / Mass : 0.0f; IsSleeping = false; SleepTimer = 0.0f;
        }
    }
};
