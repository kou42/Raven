    ...
    float& outFraction, math::Vec3& outNormal)
{
    ...
    if (std::abs(denominator) <= 1.0e-8f)
    {
        ...
        outFraction = 0.0f; outNormal = normal; return true;
    }
    ...
    for (auto [sphereEntity, sphereTransform, sphereCollider] : scene.View<TransformComponent, ColliderComponent>())
    {
        ...
        for (auto [planeEntity, planeTransform, planeCollider] : scene.View<TransformComponent, ColliderComponent>())
        {
            ...
            if (GenerateSpherePlaneManifold(sphereEntity, sphereTransform, sphereCollider,
                planeEntity, planeTransform, planeCollider, manifold)) m_Manifolds.push_back(manifold);
        }
    }
}
    ...
    float maxFraction, PhysicsRayCastHit& outHit)
{
    ...
    bool hasHit = false; float closestFraction = maxFraction; PhysicsRayCastHit closest{};
    m_BroadPhase.RayCast(scene, origin, direction, maxFraction,
        [&](Entity entity, uint32_t proxyId, float fatFraction, const math::Vec3& fatNormal, float currentMax) -> float
        {
            ...
            {
                hasHit = true; closestFraction = fraction; closest.HitEntity = entity;
                closest.Fraction = fraction; closest.Point = origin + direction * fraction; closest.Normal = normal;
            }
...
void PhysicsWorld::QueryAABB(Scene& scene, const AABB& queryBounds, std::vector<Entity>& outEntities)
{
    ...
    m_BroadPhase.QueryAABB(scene, queryBounds,
        [&](Entity entity, uint32_t proxyId) -> bool
        {
            ...
            AABB tightBounds{};
            if (ComputeColliderAABB(*transform, *collider, tightBounds) && tightBounds.Overlaps(queryBounds)) outEntities.push_back(entity);
            return true;
        });
}
...
{
    // 以前はManifoldごとに完結する単発Solverでした。
    // 現在は全ManifoldをまとめてGauss-Seidel反復することで、積み重なったBody間へ
    // Impulseを複数回伝播させます。
    SolveContactManifolds(scene, m_Manifolds, dt);
}
...
void PhysicsWorld::UpdateSleeping(Scene& scene, float dt)
{
    for (auto [entity, rigidBody] : scene.View<RigidBodyComponent>())
    {
        ...
        if (rigidBody.LinearVelocity.LengthSq() <= threshold * threshold)
        {
            ...
            {
                rigidBody.IsSleeping = true; rigidBody.LinearVelocity = math::Vec3{}; rigidBody.SleepTimer = requiredSleepTime;
            }
        }
...
void PhysicsWorld::ClearForces(Scene& scene)
{
    for (auto [entity, rigidBody] : scene.View<RigidBodyComponent>())
    {
        static_cast<void>(entity); rigidBody.Force = math::Vec3{}; rigidBody.Torque = math::Vec3{};
    }
}
...
{
    if (dt <= 0.0f) return;
    ApplyForces(scene, dt); IntegrateVelocities(scene, dt); IntegratePositions(scene, dt);
    DetectCollisions(scene); SolveCollisions(scene, dt); UpdateSleeping(scene, dt); ClearForces(scene);
}
