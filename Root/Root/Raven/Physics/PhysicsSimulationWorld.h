...
// SPH / PBF / FLIPなど具体的なSolverやParticle所有権はParticipant側へ残し、Worldは
// Fixed Step実行順序、Coupling Phase、Application frame末尾の出力同期を統括します。
class FluidWorld
{
public:
    ...
    // Scene統合時の正式入口です。
    // Participant Simulation完了後、同じFixed Step内でStatic -> Dynamic Rigidの順にCouplingを解決します。
    void StepSimulation(
        Scene& scene,
