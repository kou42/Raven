...
inline bool UIScrollView::EnsureVisible(UIElement* element, float margin)
{
    ...
    const math::Vec2 elementSize = element->GetSize();
...
inline bool UIScrollView::EnsureVisible(const math::Vec2& contentMin, const math::Vec2& contentMax, float margin)
{
    ...
    const float visibleMinX = m_ScrollOffset.x + safeMargin;
    const float visibleMaxX = m_ScrollOffset.x + viewportSize.x - safeMargin;
    if (rectMin.x < visibleMinX)
    {
        requested.x = rectMin.x - safeMargin;
    }
    else if (rectMax.x > visibleMaxX)
    {
        requested.x = rectMax.x - viewportSize.x + safeMargin;
    }

    const float visibleMinY = m_ScrollOffset.y + safeMargin;
    const float visibleMaxY = m_ScrollOffset.y + viewportSize.y - safeMargin;
    if (rectMin.y < visibleMinY)
    {
        requested.y = rectMin.y - safeMargin;
    }
    else if (rectMax.y > visibleMaxY)
    {
        requested.y = rectMax.y - viewportSize.y + safeMargin;
    }
