...
Scope<UIElement> UIElement::DetachChild(UIElement* child)
{
    ...
    auto iterator = std::find_if(
        ...
        [child](const Scope<UIElement>& candidate) { return candidate.get() == child; });
...
{
    // '/' はPath区切りとして予約します。名前に含めるとBinding文字列を一意に解釈できないため拒否します。
...
UIElement* UIElement::FindByPath(const std::string& path)
{
    return const_cast<UIElement*>(static_cast<const UIElement*>(this)->FindByPath(path));
}

const UIElement* UIElement::FindByPath(const std::string& path) const
{
    if (path.empty())
    {
        return this;
    ...
    while (begin < path.size())
    {
        ...
        for (const auto& child : current->m_Children)
        {
            ...
            {
                // 同名Siblingが存在するPathは安定Bindingになりません。曖昧な先頭要素を採用せず失敗させます。
                if (matched != nullptr)
                {
                    return nullptr;
                }
...
void UIElement::SetVisible(bool value) { if (m_Visible != value) { m_Visible = value; InvalidateMeasure(); } }
void UIElement::SetLayoutMode(UILayoutMode value) { if (m_LayoutMode != value) { m_LayoutMode = value; InvalidateMeasure(); } }
void UIElement::SetHorizontalAlignment(UIAlignment value) { if (m_HorizontalAlignment != value) { m_HorizontalAlignment = value; InvalidateArrange(); } }
void UIElement::SetVerticalAlignment(UIAlignment value) { if (m_VerticalAlignment != value) { m_VerticalAlignment = value; InvalidateArrange(); } }
...
void UIElement::OnBuildDrawList(UIDrawList& drawList, const math::Vec2& absolutePosition) const { static_cast<void>(drawList); static_cast<void>(absolutePosition); }
...
void UIElement::MeasureRecursive()
{
    if (m_Visible == false)
    {
        m_DesiredSize = math::Vec2(0.0f, 0.0f); m_MeasureDirty = false; m_ArrangeDirty = true; return;
    }
    for (auto& child : m_Children) { if (child != nullptr && child->m_MeasureDirty == true) { child->MeasureRecursive(); } }

    math::Vec2 content(0.0f, 0.0f); uint32_t count = 0u;
    for (const auto& child : m_Children)
    {
        if (child == nullptr || child->m_Visible == false) { continue; }
        ++count; const math::Vec2 childOuter = child->GetDesiredSizeWithMargin();
    ...
    content.x += m_Padding.Left + m_Padding.Right; content.y += m_Padding.Top + m_Padding.Bottom;
    m_DesiredSize = ClampSize(math::Vec2(std::max(m_PreferredSize.x, content.x), std::max(m_PreferredSize.y, content.y)));
    m_MeasureDirty = false; m_ArrangeDirty = true;
}
...
void UIElement::ArrangeRecursive(const math::Vec2& position, const math::Vec2& arrangedSize)
{
    ...
    for (auto& child : m_Children)
    {
        ...
        math::Vec2 childSize = child->m_DesiredSize; math::Vec2 childPosition = child->m_Position;
        if (m_LayoutMode == UILayoutMode::Vertical)
        {
            ...
            childPosition.y = cursorY + child->m_Margin.Top; cursorY += childSize.y + child->m_Margin.Top + child->m_Margin.Bottom + m_Spacing;
        }
...
{
    m_Context = context;
    for (auto& child : m_Children) { if (child != nullptr) { child->SetContextRecursive(context); } }
}
