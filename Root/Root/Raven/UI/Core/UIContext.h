...
private:
    friend class UIElement; void UpdateHoverTarget(UIElement* target); void UpdatePressedTarget(UIElement* target); void CollectFocusableElements(UIElement* root,std::vector<UIElement*>& outElements)const; void OnSubtreeRemoving(UIElement* subtreeRoot); static bool IsElementInSubtree(const UIElement* element,const UIElement* subtreeRoot);
private:
    math::Vec2 m_ViewportSize{}; UIDrawList m_DrawList; Scope<UIElement> m_RootElement; Scope<UIRenderer> m_Renderer; UIElement* m_HoveredElement=nullptr; UIElement* m_PressedElement=nullptr; UIElement* m_MouseCaptureElement=nullptr; UIElement* m_FocusedElement=nullptr; bool m_FrameActive=false;
};
} // namespace Raven
