...
SkeletalMeshDeformer* GetSkeletalDeformer(const RuntimeSkinnedPrimitive& primitive, std::string* errorMessage)
{
...
bool SkeletonsMatch(const Skeleton& lhs, const Skeleton& rhs, std::string* errorMessage)
{
    ...
    for (std::size_t boneIndex = 0u; boneIndex < lhs.GetBoneCount(); ++boneIndex)
    {
        ...
        {
            return SetError(errorMessage, "BlendTree SkeletonのBone順序/階層が一致しません。bone=" + std::to_string(boneIndex));
        }
    }
    return true;
}
...
SkinnedBlendTreeRuntime::SkinState* SkinnedBlendTreeRuntime::FindSkinState(std::size_t skinIndex)
{
    const auto it = std::find_if(m_SkinStates.begin(), m_SkinStates.end(), [skinIndex](const SkinState& state)
    {
...
const SkinnedBlendTreeRuntime::SkinState* SkinnedBlendTreeRuntime::FindSkinState(std::size_t skinIndex) const
{
    const auto it = std::find_if(m_SkinStates.begin(), m_SkinStates.end(), [skinIndex](const SkinState& state)
    {
        return state.SkinIndex == skinIndex;
...
const SkinnedBlendTreeRuntime::RuntimeClip* SkinnedBlendTreeRuntime::FindClip(const SkinState& state, const std::string& animationName) const
{
    const auto it = std::find_if(state.Clips.begin(), state.Clips.end(), [&animationName](const RuntimeClip& clip)
    {
...
bool SkinnedBlendTreeRuntime::AttachFromGlb(const std::string& filePath, SkinnedMeshRuntimeAsset& targetAsset, std::string* errorMessage)
{
    if (errorMessage != nullptr)
    {
        errorMessage->clear();
    ...
    {
        const auto existing = std::find_if(states.begin(), states.end(), [&primitive](const SkinState& state)
        {
...
bool SkinnedBlendTreeRuntime::GetAnimationNames(std::size_t skinIndex, std::vector<std::string>& outNames, std::string* errorMessage) const
{
    if (errorMessage != nullptr)
    {
        errorMessage->clear();
...
bool SkinnedBlendTreeRuntime::Configure(std::size_t skinIndex, const LocomotionBlendTreeConfig& config, std::string* errorMessage)
{
    if (errorMessage != nullptr) { errorMessage->clear(); }
    SkinState* state = FindSkinState(skinIndex);
    if (state == nullptr) { return SetError(errorMessage, "指定SkinIndexのBlendTree Runtime Stateがありません"); }
    if (config.IdleAnimationName.empty() || config.WalkAnimationName.empty() || config.RunAnimationName.empty())
    { return SetError(errorMessage, "Idle / Walk / Run Animation名は空にできません"); }
    if (std::isfinite(config.IdleThreshold) == false || std::isfinite(config.WalkThreshold) == false || std::isfinite(config.RunThreshold) == false
        || config.IdleThreshold < 0.0f || config.WalkThreshold <= config.IdleThreshold || config.RunThreshold <= config.WalkThreshold)
    { return SetError(errorMessage, "BlendTree Thresholdは 0 <= Idle < Walk < Run を満たす必要があります"); }
    ...
    if (idleClip == nullptr) { return SetError(errorMessage, "Idle Animationが見つかりません: " + config.IdleAnimationName); }
    if (walkClip == nullptr) { return SetError(errorMessage, "Walk Animationが見つかりません: " + config.WalkAnimationName); }
    if (runClip == nullptr) { return SetError(errorMessage, "Run Animationが見つかりません: " + config.RunAnimationName); }
    ...
    if (blendTree == nullptr) { return SetError(errorMessage, "BlendTree1Dの生成に失敗しました"); }
...
bool SkinnedBlendTreeRuntime::SetMovementSpeed(std::size_t skinIndex, float movementSpeed, std::string* errorMessage)
{
    if (errorMessage != nullptr) { errorMessage->clear(); }
    if (std::isfinite(movementSpeed) == false || movementSpeed < 0.0f) { return SetError(errorMessage, "Movement Speedは0以上の有限値である必要があります"); }
    SkinState* state = FindSkinState(skinIndex);
    if (state == nullptr) { return SetError(errorMessage, "指定SkinIndexのBlendTree Runtime Stateがありません"); }
    if (state->Configured == false || state->LocomotionTree == nullptr) { return SetError(errorMessage, "指定SkinIndexのBlendTreeがConfigureされていません"); }
    state->MovementSpeed = movementSpeed;
    if (state->AnimatorInstance.SetCurrentBlendParameter(movementSpeed) == false) { return SetError(errorMessage, "Animator BlendTree Parameterの更新に失敗しました"); }
    return true;
}

bool SkinnedBlendTreeRuntime::SetPlaybackSpeed(std::size_t skinIndex, float playbackSpeed, std::string* errorMessage)
{
    if (errorMessage != nullptr) { errorMessage->clear(); }
    if (std::isfinite(playbackSpeed) == false) { return SetError(errorMessage, "Playback Speedは有限値である必要があります"); }
    SkinState* state = FindSkinState(skinIndex);
    if (state == nullptr) { return SetError(errorMessage, "指定SkinIndexのBlendTree Runtime Stateがありません"); }
...
bool SkinnedBlendTreeRuntime::GetDebugInfo(std::size_t skinIndex, BlendTree1DDebugInfo& outInfo, std::string* errorMessage) const
{
    if (errorMessage != nullptr) { errorMessage->clear(); }
    const SkinState* state = FindSkinState(skinIndex);
    if (state == nullptr) { return SetError(errorMessage, "指定SkinIndexのBlendTree Runtime Stateがありません"); }
    if (state->Configured == false || state->LocomotionTree == nullptr) { return SetError(errorMessage, "指定SkinIndexのBlendTreeがConfigureされていません"); }
    if (state->LocomotionTree->GetDebugInfo(state->MovementSpeed, outInfo) == false) { return SetError(errorMessage, "BlendTree DebugInfoの取得に失敗しました"); }
    return true;
}

bool SkinnedBlendTreeRuntime::ApplyPoseToSkin(const SkinState& state, std::string* errorMessage)
{
    if (m_TargetAsset == nullptr) { return SetError(errorMessage, "BlendTree RuntimeがSkinnedMeshRuntimeAssetへAttachされていません"); }
    const Skeleton* animatorSkeleton = state.AnimatorInstance.GetSkeleton();
    if (animatorSkeleton == nullptr) { return SetError(errorMessage, "BlendTree Animator Skeletonがnullptrです"); }
    const SkeletonPose& pose = state.AnimatorInstance.GetCurrentSkeletonPose();
    if (pose.GetBoneCount() != animatorSkeleton->GetBoneCount()) { return SetError(errorMessage, "BlendTree評価済みPoseのBone数がSkeletonと一致しません"); }
    bool found = false;
    for (const RuntimeSkinnedPrimitive& primitive : m_TargetAsset->GetPrimitives())
    {
        if (primitive.SkinIndex != state.SkinIndex) { continue; }
        SkeletalMeshDeformer* deformer = GetSkeletalDeformer(primitive, errorMessage);
        if (deformer == nullptr) { return false; }
        if (SkeletonsMatch(*animatorSkeleton, deformer->GetSkeleton(), errorMessage) == false) { return false; }
        deformer->GetPose() = pose;
        found = true;
    }
    if (found == false) { return SetError(errorMessage, "指定SkinIndexを参照するRuntime Primitiveがありません"); }
    return true;
}

bool SkinnedBlendTreeRuntime::Update(float deltaTime, std::string* errorMessage)
{
    if (errorMessage != nullptr) { errorMessage->clear(); }
    if (m_TargetAsset == nullptr) { return SetError(errorMessage, "BlendTree RuntimeがSkinnedMeshRuntimeAssetへAttachされていません"); }
    if (std::isfinite(deltaTime) == false || deltaTime < 0.0f) { return SetError(errorMessage, "deltaTimeは0以上の有限値である必要があります"); }
    for (SkinState& state : m_SkinStates)
    {
        if (state.Configured == false) { continue; }
        state.AnimatorInstance.Update(deltaTime);
        if (ApplyPoseToSkin(state, errorMessage) == false) { return false; }
    }
