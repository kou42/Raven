...
inline bool Run(std::string* errorMessage = nullptr)
{
    ...
    if (Detail::Expect(dash.IsActive(), "Dashが押下Edgeで開始されませんでした", errorMessage) == false)
    {
        return false;
    }
    ...
    if (Detail::Expect(
            Detail::NearlyEqual(firstVelocity.x, expectedComponent)
                && Detail::NearlyEqual(firstVelocity.z, expectedComponent),
    ...
    if (Detail::Expect(
            Detail::NearlyEqual(heldVelocity.x, firstVelocity.x)
                && Detail::NearlyEqual(heldVelocity.z, firstVelocity.z),
            "Dash中に方向が変更されました",
            errorMessage) == false)
    {
        return false;
    ...
    if (Detail::Expect(dash.IsActive() == false, "Dash Hold中に再発火しました", errorMessage) == false)
    {
        return false;
    }
    ...
    if (Detail::Expect(dash.IsActive(), "Cooldown終了後にDashを再開できませんでした", errorMessage) == false)
    {
        return false;
    }
    ...
    if (Detail::Expect(
            Detail::NearlyEqual(fallbackVelocity.x, 0.0f)
                && Detail::NearlyEqual(fallbackVelocity.z, -config.Speed),
    ...
    return Detail::Expect(dash.IsActive() == false, "AirborneでGroundedOnly Dashが開始されました", errorMessage);
}
